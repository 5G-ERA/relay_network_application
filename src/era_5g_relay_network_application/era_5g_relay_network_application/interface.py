...
# TODO make a class encapsulating set and its lock
...
def connect_data(sid, environ):
    ...
    """
    print(
        f"Connected data. Session id: {sio.manager.eio_sid_from_sid(sid, '/data')}, namespace_id: {sid}"
    )
...
def connect_control(sid, environ):
    ...
    print(
        f"Connected control. Session id: {sio.manager.eio_sid_from_sid(sid, '/data')}, namespace_id: {sid}"
    )
...
def connect_results(sid, environ):
    ...
    print(
        f"Connected results. Session id: {sio.manager.eio_sid_from_sid(sid, '/data')}, namespace_id: {sid}"
    )
...
def json_callback_websocket(sid, data):
    ...
    """
    logging.debug(
        f"client with task id: {sio.manager.eio_sid_from_sid(sid, '/data')} sent data {data}"
    )
    print(data)
    global workers
    ...
    if packet_type == PacketType.MESSAGE:
        ...
        proxy = rospy.ServiceProxy(
            packet.service_name, ros_loader.get_service_class(packet.service_type)
        )
        ...
        if args:
            ...
            if sr:
                with subscribers_lock:
                    result_subscribers.add(
                        sio.manager.eio_sid_from_sid(sid, "/control")
                    )
...
def main():
    ...
    for topic_name, _, topic_type in topics:
        _ = WorkerResults(
            topic_name, topic_type, sio, result_subscribers, subscribers_lock
        )
