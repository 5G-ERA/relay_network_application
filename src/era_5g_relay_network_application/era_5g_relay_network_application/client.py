...
def main(args=None) -> None:
    ...
    for topic_name, topic_name_remapped, topic_type, compression in topics_incoming_list:
        queue = Queue(QUEUE_LENGTH_TOPICS)
    ...
    for service_name, service_type in services:
        request_q = Queue(QUEUE_LENGTH_SERVICES)
        response_q = Queue(QUEUE_LENGTH_SERVICES)
        worker = WorkerServiceServer(service_name, service_type, request_q, response_q, node)
        services_workers[service_name] = worker
    ...
    try:
        ...
        for topic_name, topic_name_remapped, topic_type, compression in topics_outgoing_list:
            ...
            q = Queue(QUEUE_LENGTH_TOPICS)
            channel_type = get_channel_type(compression, topic_type)
            if channel_type in IMAGE_CHANNEL_TYPES:
                worker = WorkerImageSubscriber(topic_name, topic_type, node, q)
                send_function = partial(send_image, event=f"topic/{topic_name}", client=client, channel_type=channel_type, can_be_dropped=True)
            else:
                worker = WorkerSubscriber(topic_name, topic_type, compression, node, q)
                send_function = partial(client.send_data, event=f"topic/{topic_name}", channel_type=channel_type, can_be_dropped=True)
        ...
        for service_name, service_worker in services_workers.items():
            ...
            send_function = partial(client.send_data, event=channel_name, can_be_dropped=False)
            worker_socketio = WorkerSocketIO(service_worker.request_queue, send_function)
            worker_socketio.daemon = True
            worker_socketio.start()
            socketio_workers.append(worker_socketio)
        ...
        if transforms_to_listen:
            ...
            send_function = partial(client.send_data, event="topic//tf")
            worker_socketio = WorkerSocketIO(tf_queue, send_function)
            worker_socketio.daemon = True
            worker_socketio.start()
            socketio_workers.append(worker_socketio)
